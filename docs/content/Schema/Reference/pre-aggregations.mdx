...
Pre-aggregations can be configured to behave in real-time fashion by joining an
existing pre-aggregation with fresh data retrieved from the data source. This
can be done by setting `unionWithSourceData` to `true`:
...
pretty same performance from the Cube Store side but multiple times less cost
