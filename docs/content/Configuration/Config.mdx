...
Cube can be configured both via environment variables and by providing
...
> ✏️ Called **once** per [`appId`][self-opts-ctx-to-appid]
...
If no option is passed, Cube will use the environment variable `CUBEJS_DB_TYPE`
to resolve `dbType`.
...
> ✏️ Called **once** per [`dataSource`][ref-schema-ref-datasource]
...
> ✏️ Called on **every** GraphQL/REST/WebSockets API request
...
> ✏️ Called on **every** SQL API request
...
> ✏️ Called on **every** SQL API request for a SQL user change
...
Schema version is used to tell Cube that the schema should be recompiled in case
the schema depends on dynamic definitions fetched from an external database or
API. This method is called on each request, however the `RequestContext`
parameter is reused per application ID as determined by
...
freshness every 30 seconds. Please consult the [`refreshKey`
documentation][ref-pre-aggregations-refresh-key] on how to set refresh intervals
...
Option to extend the `RequestContext` with custom values. Can be an asynchronous
function.
...
memory by resetting their `maxCompilerCacheKeepAlive` every time they are
accessed.
...
issued. This statistics is used solely for the purpose of constant improvement.
...
You can pass this object to set advanced options for Cube's Query Orchestrator.
