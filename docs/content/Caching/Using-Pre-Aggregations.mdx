...
## Querying real-time data with rollup unions

By default, Cube is configured to build and refresh pre-aggregations in the
background and use them for query results. However, this approach can lead to
stale data being returned when a query is made against a pre-aggregation that
has not yet been refreshed. To avoid this, a pre-aggregation can be configured
to act as a **rollup union**, which can query data from the source database and
join it with existing data from a pre-aggregation. This allows leveraging
pre-aggregations for scenarios that have real-time data requirements. To enable
this for a pre-aggregation, set
[`unionWithSourceData`][ref-schema-ref-preaggs-unionwithsrcdata] to `true` in
...
[ref-schema-ref-preaggs-unionwithsrcdata]:
  /schema/reference/pre-aggregations#parameters-union-with-source-data
